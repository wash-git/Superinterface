...
function fInit () {
	...
	chmod $CPPERMFIXOS $CPNOMECIDADES
...
function fParq () {
	...
	cat $CPNOMECIDADES | sed 'y/áÁàÀãÃâÂéÉêÊíÍóÓõÕôÔúÚçÇ/aAaAaAaAeEeEiIoOoOoOuUcC/' | awk 'BEGIN{FS=","}{print "grep -Howi \""$1"\" '$CPTEMP'/*.txt.'$CPACENTO'.'$CPCONTRL'.'$CPMAIUSCULA' | awk '\''BEGIN{FS=\":\"}{print $2\", \"$1}'\'' | sort | uniq -c | sed '\''s/\\, /\\,/g'\'' | sed '\''s/   /  /g'\''| sed '\''s/  / /g'\'' | sed '\''s/  //g'\'' | awk '\''{guarda=$1; $1=\"\"; print $0\",\"guarda;}'\''"}' > $CPPADMIN/super_temp_executa_conta_ocorrencias_novospdf.bash 2>/dev/null
	if [ $? -ne 0 ]; then
		mv -f $CPTEMP/*.pdf $CPQUARENTINE/.			# deixar os arquivos PDF na quarentena para posterior análise
		fMens "$FInsuc" "$MErr14"
		exit
