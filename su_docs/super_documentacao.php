...
<p>Essa reunião de documentos apoiada pela Superinterface deveria ter por base alguma característica comum definida por seus usuários, embora a solução exija poucos formalismos de classificação e proveniência dos documentos. Isso tudo para possibilitar o máximo de fluidez na gestão dos documentos por parte dos curadores.</p>

<p>A recuperação da informação sempre foi uma questão delicada para a ciência da informação. Neste sentido, a Superinterface oferece facilidades de indexação e busca de termos na coleção de documentos, permitindo a criação de arquiteturas de acordo com as necessidades da aplicação. São muito amplas as possibilidades de indexação dos conteúdos dos arquivos presentes no acervo. No momento da instalação deste pacote, as seguintes informações estarão automaticamente disponíveis para serem buscadas nos conteúdos dos arquivos:</p>
<dl>
...
<p>Os dados estão organizados por documento. Ferramentas de automatização estão continuamente sendo avaliadas e utilizadas para que a Superinterface consiga fazer essa coleta de dados, inclusive:</p>
<dl>
...
<p>Destas 3 abordagens, a mais madura é a primeira e já vem sendo utilizada amplamente para gerar parte dos metadados da plataforma Superinterface.</p>
<!--  ******************************************* -->
...
<p>Assim, recomenda-se fortemente a realização dessa primeira experiência de funcionamento da Superinterface antes de se implementar a modelagem específica da aplicação do usuário.  É uma forma prática de verificar que todo ambiente foi configurado corretamente e a aplicação Superinterface está em pleno funcionamento.</p>
<p>A seguir iremos descrever estas etapas e esperamos que ela possa ser útil. Uma boa leitura!</p>
<!--
 ..................................................................................................
 -->
<li><h3 id="arquitetura">Arquitetura da Solução</h3></li>
<p>A figura abaixo representa os blocos que compõem a Superinterface: </p>
...
<pre>
MAILTO=""
*/1  *  *  *  *  SHELL=/bin/bash; cd /var/www/html/host1/su_install/; ./super_novospdf.sh cron
</pre>
Dicas e observações:
<ul><li>no caso exemplificado, o vigilante está sendo ativado a cada 1 minuto.</li>
<li>estamos garantindo a utilização do BASH, em vez do default /bin/sh ("dash").</li>
...
<p></p><p></p>
<p>Pronto. Agora já se pode ter a primeira experiência de funcionamento da Superinterface, a qual será descrita na próxima seção deste documento.</p>
<p></p>
<!--
 ..................................................................................................
-->
<li><h3 id="experiencia">Primeira Experiência</h3></li>
<p>Após se ter o ambiente do servidor preparado, conforme descrito na seção <a href="#preparacao_ambiente" title="Ambiente do Servidor">Ambiente do Servidor</a>, é hora de se ter uma primeira experiência de funcionamento da Superinterface.  Vamos conhecer a Superinterface funcionando de verdade.</p>
<p>O pacote da Superinterface já vem completo para proporcionar uma primeira experiência de seu funcionamento. Para isto, é necessário apenas dois paços:</p>
...
<p>Acompanhe o log da instalação através do arquivo super_logshell.log que estará na pasta su_logs/.</p>
<p>Apenas isso. Depois é passar a se certificar que realmente tudo fora realizado durante a instalação e, em seguida,  passar a fazer os primeiros usos da Superinterface como mostra o roteiro a seguir: </p>
<ol style="list-style-type:lower-alpha">
...
<ul>
  ...
  <li><strong> su_temp:</strong> pasta de arquivos temporários do usuário.</li>
  <li><strong> su_uploads:</strong> pasta para onde serão direcionados os arquivos PDF após seu upload pelo usuário, com objetivo de fazer incorporações ao acervo da aplicação Superinterface.  A execução deste processo de incorporação de novos documentos PDF ao acervo estará sob responsabilidade do vigilante (ativado pelo cron).</li>
...
<p>A modelagem da base de dados é uma tarefa crucial para o bom funcionamento da Superinterface. A partir desta modelagem será possível realizar as seguintes operações:</p>
<ul>
...
<p>Assim, preparar a base de dados obedecendo algumas regras é fator crucial para sucesso das operações da Superinterface. Regras estas que estaremos descrevendo a partir de agora.</p>
<ol style="list-style-type:lower-alpha">
<li>Nome das Tabelas</li>
<p>Para que seja possível gerar automaticamente os códigos PHP da Plataforma faz-se necessário que o banco de dados tenha algumas características especiais.  As tabelas devem ter seus nomes no plural, para indicar o carater coletivo dos dados de uma tabela (conjunto de registros). Na modelagem fornecida com o pacote, são exemplos de tabelas existentes: su_documents, su_tiposdocumentos e su_cidades.</p>

<p>Não utilizaremos o uso de maiúsculas na primeira letra do nome da tabela em função da existência de questões associadas à transição dos nomes nas várias versões do MySQL, com diferentes configurações de sensibilidade de case. Portanto, por simplicidade adotou-se a prática de se usar letras minúsculas para todos os nomes, sejam de campos ou de tabelas.</p>
<li>Nome dos Campos das Tabelas</li>
<p>Atenção especial deve-se ter com essas regras de nomeação dos campos das tabelas:</p>
<ul>
...
<tr><td>INSERTs em tabelas</td><td>Conjunto de planilhas .CSV com informações para inserção na base de dados</td><td>Opcional</td></tr>
<tr><td>INSERTs em tabelas</td><td>Conjunto de arquivos .SQL com regras e informações para  inserção na base de dados.</td><td>Opcional</td></tr>
<tr><td>Script iniciação</td><td>Script SHELL muito útil no uso com Expressões Regulares (ERs), melhorando as inserções na base de dados</td><td>Opcional</td></tr>
...
<p>Este arquivo possui três partes, chamadas "Parte-1", "Parte-2" e "Parte-3", cada uma contendo um conjunto de parâmetros. Recomendamos que só sejam alterados os parâmetros da primeira parte, deixando os parâmetros das demais partes sempre inalterados.</p>
<ol style="list-style-type:lower-alpha">
...
<p>Para possibilitar o perfeito funcionamento da Superinterface, inclusive disponibilizar a facilidade de busca de verbetes nos conteúdos do acervo de arquivos, é necessário alimentar algumas de suas tabelas com informações oriundas de fontes externas. Isso possibilitará a construção do necessário vocabulário controlado da solução.</p>

<p>Para facilitar a instalação, ao se baixar o pacote da Superinterface um conjunto completo de informações que constittui uma Base de Informações inicial já estará disponível na pasta su_info/. Esse conjunto de informações é aderente à modelagem trazida pelo pacote.  Assim, para uma primeira experiência da Superinterface não há necessidade de qualquer alteração nestes arquivos.  Entretanto, para uma outra modelagem da base de dados será necessário trocar estes arquivos em função das novas necessidades de informações.</p>

<p>Informações para constituir essa Base de Informações podem (e devem) ser fornecidas pelos usuários, conforme as necessidades da modelagem da base de dados realizada. Essa Base de Informações será automaticamente inserida na base de dados pelas rotinas da Superinterface, seja no momento da instalação da solução ou em qualquer outro momento de sua utilização dependendo da fonte de informação. Este processo de alimentação da base de dados tem uma certa sequência de realização predefinida, conforme a tabela explicativa a seguir:</p>
<table class="tablec1">
...
<p>Ou seja, no procedimento de instalação a Superinterface primeiro irá procurar o arquivo contendo comandos SQL de criação de tabelas, depois procurará o conjunto de arquivos de planilha CSV e fará a inserção de seus conteúdos na base de dados, seguido da busca do conjunto de outros arquivos .SQL e, por último, buscará executar um script shell do usuário com mais comandos de inserção de dados.   De forma assíncrona, após passado o momento da instalação, o usuário poderá acessar o Portal de Administração da Superinterface e adicionar metadados a base. </p>
<ol style="list-style-type:lower-alpha">
<li>Através de Planilhas (arquivos .CSV)</li>
<p>Este é o modo preferencial utilizado no momento da instalação da solução. Com certeza o modo mais fácil e prático.  O usuário responsável pela instalação deverá disponibilizar essas planilhas na pasta su_info/ e todos os arquivos presentes nesta pasta terão seus conteúdos inseridos na base de dados.  Deve ser destacado que existem exigências quanto as planilhas estarem obedecendo algumas regras especiais construtivas, conforme está explicado mais adiante.</p>

<p>Podem ser disponibilizadas tantas planilhas quanto forem necessárias. Isto é, tantas quanto a modelagem da base de dados necessitar. As seguintes planilhas já são fornecidas com o pacote da Superinterface e são aderentes a modelagem da base que está implementada no pacote:</p>
<table class="tablec2">
...
<p>O usuário poderá disponibilizar arquivos com instruções SQL (INSERT, UPDATE, ALTER TABLE e outros) para serem executados quando da instalação do sistema. Podem ser disponibilizados tantos arquivos quanto sejam necessários através da pasta su_info/. Rotinas da  Superinterface identificarão estes arquivos e executarão essas instruções SQL.  A Superinterface executará esses arquivos SQL por ordem alfabética dos nomes dos arquivos, um após o outro.</p>

<p>O pacote da Superinterface baixado já disponibiliza na pasta su_info/ um primeiro arquivo SQL o qual é aderente a modelagem da base de dados fornecida. Para uma outra modelagem, o usuário deverá modificar ou mesmo trocar este arquivo.</p>
<p>Obs: não é obrigatório que exista qualquer arquivo SQL no momento da instalação, exceptuando logicamente o arquivo de criação das tabelas.</p>
...
<p>Passado o momento da instalação, usuários com acesso ao Painel de Administração da Superinterface poderão se utilizar das facilidades da Plataforma Potlatch e inserirem mais informações na base de dados. Os procedimentos para utilização da Potlatch estão detalhadas no Manual do Usuário em seção específica sobre o assunto. </p>
<li>Padronização dos nomes e dos conteúdos das planilhas</li>
<p>As planilhas devem estar obedecendo a algumas regras de formação, tanto em relação aos nomes destes arquivos quanto aos seus conteúdos. O objetivo dessas regras de formação é possibilitar um processo automatizado de incorporação das informações trazidas pelas planilhas à base de dados. Processo esse que independe da quantidade de planilhas fornecidas. A seguir estão descritas estas regras de formação que devem obrigatoriamente serem seguidas.</p>
<ul>
<li>Quanto ao nome do arquivo:</li>
<p>O nome da planilha é composto de duas partes, onde o primeiro underscore encontrado faz esta separação: a primeira parte vai até o primeiro underscore (inclusive), e a segunda parte começa após este primeiro underscore. Ou seja, o primeiro underscore é um separador: a primeira parte do nome da planilha é um texto livre, e será despezado pela Superinterface; após o primeiro underscore, o texto deve corresponder exatamente ao nome de alguma tabela da base de dados. Dessa forma a Superinterface saberá automaticamente onde fazer a inserção das informações do arquivo.</p>
<p>Vejamos um exemplo, através da planilha referente aos países:</p>
<p class="centertext">supercsv-ibge_su_paises.csv</p><br />
<p>Pela regra descrita, o conteúdo deste arquivo será direcionado à tabela "su_paises".</p>
<li>Quanto ao conteúdo do arquivo:</li>
<p>O conteúdo de cada planilha deve obedecer a seguinte construção:</p>
...
<p>Todas as atividades relevantes da Superinterface, inclusive qualquer ação imprópria no tratamento do acervo, são registradas em seu arquivo de log de forma a permitir ao usuário administrador o acompanhamento destes eventos. O principal objetivo do registro de logs da Superinterface está relacionada à sua conformidade, e registrar o histórico de suas atividades. </p>

<p>Na pasta de logs desta aplicação existem dois arquivo de logs:</p>
<ol style="list-style-type:square">
<li> su_logs/super_logshell.log</li>
<li> su_logs/super_logshell.html</li>
</ol>
<p>Todo log é registrado no arquivo super_logshell.log.  Já o segundo arquivo, super_logshell.html, é apenas uma cópia do primeiro arquivo destinado a ser visualizado através da interface administrativa da Superinterface.  Mas seu conteúdo é exatamente igual ao do primeiro arquivo.</p>
<p>A Superinterface realiza a rotação de logs através de um processo automatizado, realizando a compactação do arquivo e renomeando-o. Esse processo ocorre quando o tamanho deste arquivo de log atinge um determinado valor parametrizado através do arquivo de configuração. Essa compactação gera um arquivo ".tar", o qual fica guardado na própria pasta de logs, e um novo arquivo de logs é iniciado.</p>

<p>É muito importante verificar periodicamente o arquivo de log, certificando que todas as operações estão sendo realizadas sem ocorrências de exceções.</p>
...
<p>Eventuais anormalidades no tratamento do acervo são registradas em seu arquivo de log (su_logs/super_logshell.log) de forma a permitir ao usuário administrador o acompanhamento e as intervenções de correção necessárias. Recomenda-se o acompanhamento continuado deste arquivo de log por parte do usuário administrador desta aplicação. Ou seja, o arquivo de log deve ser acompanhado não apenas durante a instalação da Superinterface, mas rotineiramente durante sua operação.</p>
<p>Durante a instalação, caso se verifique alguma dificuldade e o arquivo de log não tenha o registro completo da situação, execute alternativamente o script abaixo via comando do terminal, e acompanhe as mensagens:</p>
$ ./super_scriptinicial.sh<br />
<p>Após realizar o diagnóstico do problema e ter solucionado a dificuldade, (re)instale a solução via seu comando padrão:</p>
...
<li>Irei utilizar uma outra modelagem da base de dados.  O que preciso mudar?</li>
<ul>
<li>Crie seu arquivo com instruções SQL de criação das tabelas, que está referenciado no arquivo de configuração pela variável CPCRIATABELAS, colocando-o na pasta su_info/. Obs: o arquivo tem que ter extensão .sql</li>
<li>Crie seus próprios arquivos CSV necessários e coloque-os na pasta su_info/.</li>
<li>Crie seu arquivo com instruções SQL para realizar INSERTS de informações na base de dados (e outros comandos SQL), colocando-o na pasta su_info/.  Obs: o arquivo tem que ter extensão .sql, e pode ter qualquer nome</li>
<li>Crie seu arquivo script shell de iniciação, colocando-o na pasta su_info/. Esse arquivo é referenciado no arquivo de configuração pela variável CPINSERTSCRIPT.</li>
<li>Crie seu arquivo script shell de regime, que será executado todas as vezes que o cron ativar a Superinterface, colocando-o na pasta su_info/. Esse arquivo é referenciado no arquivo de configuração pela variável CPINSERTACERVO.</li>
<li>Se desejar uma estatística dos conteúdos de apenas parte das tabelas, a cada ativação da Superinterface, crie sua planilha com a lista das tabelas que deseja ter as informações e coloque este arquivo na pasta su_info/. Obs: este arquivo é referenciado na configuração da Superinterface pela variável CPRELATORIOTABELAS.</li>
</ul>
Verifique as regras de nomes das tabelas e arquivos, conforme explicado nas seções  <a href="#modelagem" title="Modelagem">Modelagem</a> e <a href="#baseinformacoes" title="Base de Informações">Base de Informações</a>.
  .
</ol>
</ol>
<!--  ******************************************* -->
...
<p>As buscas de conteúdos no acervo da Superinterface estão disponíveis na interface gráfica disponibilizada pelo Giramundonics e, considerando a modelagem da base de dados fornecida, pode ser realizada de três formas:</p>
<ul>
...
<p>A medida que se inicia a digitação do termo a ser buscado, as buscas já são iniciadas. Os termos que estão indexados pertencem a um vocabulário controlado, constituindo um conjunto normalizado de termos que serve à indexação e à recuperação da informação. Esses termos são predefinidos através de arquivos de configuração, os quais o administrador da Superinterface pode alterar.</p>
<br /><br />
...
<p>Sempre que um campo for alterado, ele ficará na cor vermelha indicando que aquela informação ainda não foi gravada na base de dados. Na extremidade direita desta linha da tabela encontra-se o botão ATUALIZA para salvar na base de dados as alterações realizadas.</p>

<p>Abaixo temos um recorte da tela do Giramundonics mostrando como são visualizados os campos editados pelo curador (data_doc, sigla, descricao, relevancia), e já com duas buscas realizadas automaticamente pela Superinterface (busca por instituições citadas no documento e busca de signatário do documento):</p>
